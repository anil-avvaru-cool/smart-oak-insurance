...
def validate_graph_features(claims_df: pd.DataFrame) -> None:
    ...
    if "graph_hop_distance" in claims_df.columns:
        ...
        if sentinel_pct > 50:
            print(f"  ❌ >50% sentinel values indicate graph is not connected to fraud entities")
        elif sentinel_pct > 20:
            print(f"  ⚠️ {sentinel_pct:.1f}% sentinel values may indicate incomplete graph coverage")
        else:
            print(f"  ✅ graph connectivity looks healthy")
        ...
        print("  ⚠️ graph_hop_distance column not found in dataset")
    ...
    if "attorney_centrality_score" in claims_df.columns:
        ...
        if (centrality_vals < 0).any() or (centrality_vals > 1.0).any():
            print(f"  ❌ attorney centrality score out of expected [0, 1] range")
        else:
            print(f"  ✅ attorney centrality scores in valid range")
    ...
    if "shared_attribute_count" in claims_df.columns:
        ...
        if (shared_count < 0).any():
            print(f"  ❌ negative shared attribute counts found")
        else:
            print(f"  ✅ shared attribute counts are valid")
...
def validate_graph_topology(neo4j_uri: str | None = None, neo4j_user: str | None = None, neo4j_password: str | None = None) -> None:
    ...
    try:
        ...
        with driver.session() as session:
            ...
            if total_nodes == 0:
                print(f"  ❌ GRAPH IS EMPTY - no nodes found in Neo4j")
            ...
            if rel_count == 0:
                print(f"  ❌ NO RELATIONSHIPS found - graph is disconnected")
            else:
            ...
            if fraud_count == 0:
                print(f"  ❌ NO fraud claimants in graph (explains 999 sentinel values)")
            elif fraud_count < 5:
                print(f"  ⚠️ very few fraud claimants ({fraud_count}), may limit path discovery")
            else:
                print(f"  ✅ fraud claimants present for path analysis")
            ...
            if isolated > 0:
                print(f"  ⚠️ {isolated} isolated claimant nodes (no SHARES relationships)")
            else:
                print(f"  ✅ all claimants have relationships")
        
        driver.close()
    except Exception as e:
        print(f"  ❌ error validating graph topology: {e}")
    ...
    if neo4j_uri and neo4j_user and neo4j_password:
        ...
        if "graph_hop_distance" in claims_df.columns:
            ...
            if sentinel_pct == 100:
                ...
                print("   Step 1: ✅ Generate data (claims have synthetic graph_hop_distance)")
                print("   Step 2: ❓ Build graph (check Neo4j status above)")
                print("   Step 3: ❌ Compute graph features (NOT DONE - run this command:)")
